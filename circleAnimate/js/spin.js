...
//    console.info("x:" + radius*Math.sin((i+1)*deg*Math.PI/180) + " y:" + radius*Math.cos((i+1)*deg*Math.PI/180));
  }

 /* console.info("centerX:" + centerX +" centerY:" + centerY);
  console.info("degree:" + deg);
  console.info("radius:" + radius);
  console.info("clientHeight:" + base.clientHeight + " clientWidth:" + base.clientWidth);
  for(x in points){
    console.info("x:" + points[x].x + " y:" + points[x].y);
  }
...
//    element.style.setProperty("-webkit-transition", "all 0.2s");
//    element.style.setProperty("-moz-transition", "all 0.2s");
//    element.style.setProperty("-o-transition", "all 0.2s");
  });
...
//    var currentd = d;
    item.forEach(function (element, index, array) {
      element.style.left = (centerX + radius*Math.sin(index*d*Math.PI/180 + speed/1000) - itemSize/2) + "px";
      element.style.top = (centerY - radius*Math.cos(index*d*Math.PI/180 + speed/1000) - itemSize/2) + "px";
//      element.style.setProperty("transform", "translate("+ (centerX + radius*Math.sin(index*d*Math.PI/180 + speed/1000) - itemSize/2) + "px" + ","+
//          (centerY - radius*Math.cos(index*d*Math.PI/180 + speed/1000) - itemSize/2) + "px" +")");
//      element.style.webkitTransform = 'translateX(' +  (centerX + radius*Math.sin(index*d*Math.PI/180 + speed/1000) - itemSize/2) + 'px)';
//      element.style.webkitTransform = 'translateY(' + (centerY - radius*Math.cos(index*d*Math.PI/180 + speed/1000) - itemSize/2) + "px)";
    });
//    d++;
    speed++;
